...
class RotaryPositionalEmbeddings(nn.Module):
    ...
    def _build_cache(self, x: torch.Tensor):
        ...
        seq_idx = torch.arange(1,seq_len+1, device=x.device).float()
    ...
    def _neg_half(self, x: torch.Tensor):
        ...
        neg_half_sin=torch.cat([-x[:, :, :, d_2:], x[:, :, :, :d_2]], dim=-1)
        neg_half_cos=torch.cat([ x[:, :, :, :d_2],x[:, :, :, d_2:]], dim=-1)

        return neg_half_sin,neg_half_cos
    ...
    def forward(self, x: torch.Tensor):
        ...
        self._build_cache(x)
        neg_half_sin_x, neg_half_cos_x = self._neg_half(x)
        size= x.shape[2]
        
        print(f"x shape: {x.shape}")
        print(f"neg_half_cos_x shape: {neg_half_cos_x.shape}")
        print(f"neg_half_sin_x shape: {neg_half_sin_x.shape}")
        print(f"cos_cached shape: {self.cos_cached.shape}")
        print(f"sin_cached shape: {self.sin_cached.shape}")
        
        # Ensure the slice is correct
        print(f"cos_cached slice shape: {self.cos_cached[:x.shape[2]].shape}")
        print(f"sin_cached slice shape: {self.sin_cached[:x.shape[2]].shape}")

        x_rope = (neg_half_cos_x * self.cos_cached[:,:,:x.shape[2],:]) + (neg_half_sin_x * self.sin_cached[:,:,:x.shape[2],:])
        return x_rope
    
        # self._build_cache(x)
        # neg_half_cos_x, neg_half_sin_x= _neg_half(x)
        # x_rope = (neg_half_cos_x * self.cos_cached[:x.shape[2]]) + (neg_half_sin_x* self.sin_cached[:x.shape[2]])
...
#         seq_idx = torch.arange(seq_len+1, device=x.device).float()
        
#         # Calculate the product of position index and θ_i
#         idx_theta = torch.einsum('n,d->nd', seq_idx, theta)
        
#         # Concatenate so that for row m we have [mθ_0, mθ_1, ..., mθ_d/2, mθ_0, mθ_1, ..., mθ_d/2]
#         idx_theta2 = torch.cat([idx_theta, idx_theta], dim=1)
...
#         # Return [-x(d/2+1), -x(d/2+2), ..., -x(d), x(1), x(2), ..., x(d/2)]
...
#         # Calculate [-x(d/2+1), -x(d/2+2), ..., -x(d), x(1), x(2), ..., x(d/2)]
#         neg_half_x = self._neg_half(x_rope)
        
#         # Calculate (x_m(i) * cos(mθ_i) - x_m(i+d/2) * sin(mθ_i), x_m(i+d/2) * cos(mθ_i) + x_m(i) * sin(mθ_i))
#         # x_rope = (x_rope * self.cos_cached[:x.shape[2]]) + (neg_half_x * self.sin_cached[:x.shape[2]])
#         x_rope = x_rope
        
#         # Concatenate back the non-rotary portion with the rotary portion if applicable
...
class GroupedQueryAttention(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.head_dim = self.n_embd // self.n_query_head  # Common head dimension

        # Key, Query, Value Projections
        self.q_proj = nn.Linear(self.n_embd, self.n_embd)  # [n_embd, n_embd]
        self.k_proj = nn.Linear(self.n_embd, self.n_kv_head * self.head_dim)  # [n_embd, n_kv_head * head_dim]
    ...
    def forward(self, x):
        ...
        # Aggregate the attention with the values
        y = einsum(att, v, 'b hkv g t_q t_k, b hkv t_k d -> b hkv g t_q d')
