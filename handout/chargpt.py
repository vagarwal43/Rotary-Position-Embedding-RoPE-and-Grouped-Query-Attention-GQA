...
if __name__ == '__main__':
    ...
    text = open('/content/drive/MyDrive/GenAI/HW-1/input.txt', 'r').read() # don't worry we won't run out of file handles
    ...
    wandb_run_id_path = os.path.join("content/drive/MyDrive/GenAI/HW-1/out/vanilla16", "wandb_run_id.txt")
    ...
    if config.model.pretrained_folder!=None:
        ...
        wandb_run_id_path = os.path.join(config.model.pretrained_folder, "wandb_run_id.txt")
    
    if os.path.exists(wandb_run_id_path):
        with open(wandb_run_id_path, "r") as f:
            run_id = f.read().strip()
        wandb.init(project="Gen-AI_HW1", name="Model_for_4.4", resume="must", id=run_id)
    else:
        # If wandb run ID is not found, start a new one
        wandb.init(project="Gen-AI_HW1", name="Model_for_4.4")
    ...
    def batch_end_callback(trainer):
        if trainer.iter_num % 1 == 0:
            ...
            if trainer.device == "cuda":
                memory_consumed = torch.cuda.memory_allocated() / (1024 * 1024)  # Convert bytes to MB
                attn_mem.append(memory_consumed)
            
            key_heads = 1
            
            wandb.log({
                "Iteration": trainer.iter_num,
                "Training Loss": trainer.loss.item(),
                "Attention Time (ms)": trainer.attn_times * 1000,
                "Memory Consumption (MB)": memory_consumed,
                "Key_Heads": key_heads
            })
        ...
        if (trainer.iter_num + 1) % 200 == 0:
            ...
            with open(os.path.join(config.system.work_dir, "wandb_run_id.txt"), "w") as f:
